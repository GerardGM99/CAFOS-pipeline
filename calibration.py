...
def init():
    global DATADIR, directory, PLOTDIR
...
def create_master_bias(summary_bias, plot = False):
    ...
    if plot is True:
        ...
        plt.savefig(os.path.join(PLOTDIR, "master_bias.png"), dpi=80)
        plt.close()
        
    return master_bias, median_bias, gain, readout_noise

def apply_master_bias(img_code='*', plot=False):
    
    ifc_all, summary_all,_ = read_images(img_code=img_code, plot=plot)
...
def create_master_flat(plot = False):
    ...
    if plot is True:
        for filename, objname, exptime, median, robust_std in summary_flat[
            ...
            plt.savefig(os.path.join(PLOTDIR, f"{filename}.png"), dpi=80)
            plt.close()
    ...
    for grism in grisms:
        for slit in slits:
            ...
            if plot is True:
                ...
                plt.savefig(os.path.join(PLOTDIR, f"master_flat_{grism}_{slit}.png"), dpi=80)
                plt.close()        
...
def sky_substraction():
    ...
    print('---')
    print('Here you need to select two empty background regions from the 2D image. They need to be at each side of the main object trace. They should not contain other traces. INTEGERS ONLY.')
        
    cont = False
    while cont is False:
        print('---')
        skyregion1_min = int(input('Skyregion1 min: '))
        skyregion1_max = int(input('Skyregion1 max: '))
        skyregion2_min = int(input('Skyregion2 min: '))
        skyregion2_max = int(input('Skyregion1 max: '))
...
def spec_align():
    ...
    print('Chose the x and the y ranges to align the spectum. The spectral trace MUST be in the window. INTEGERS ONLY')
...
def spec_extract(initial_xmin, initial_xmax, initial_ymin, initial_ymax):
    ...
    for iplot in range(2):
        ...
        tea_imshow(fig, ax, data, vmin=0, vmax=500, title=input_filename,
                   cmap='gray', aspect='auto')
        if iplot == 1:
            ax.set_ylim(initial_ymin, initial_ymax)
